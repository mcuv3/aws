	...
	process:
		for {
			...
			for _, m := range msgs {
				...
				if sent == int(req.GetBatchLimit()) {
					break process
				}

				err := stream.SendMsg(&aws.ReceiveMessageResponse{Message: &aws.Message{Id: string(m.UserMessageId), Message: m.Message, QueueName: queue.Name}})
			...
			if wait++; int(req.GetWaitLimit()) <= wait {
				break process
			}
...
func (s *SQSService) DeleteMessage(ctx context.Context, req *aws.DeleteMessageRequest) (*aws.DeleteResponse, error) {
	id := req.GetMessageId()
